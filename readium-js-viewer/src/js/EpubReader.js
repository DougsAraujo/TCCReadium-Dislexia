...
//        var KEY_RIGHT = 0x27;
    ...
    // Set handlers for click events
        ...
        bottom: null
          ...
          this.$ruler.top = $("<div id='ruler-top' class='ruler'>");
          this.$ruler.bottom = $("<div id='ruler-bottom' class='ruler'>");
            
            $("body").prepend(this.$ruler.top);
            $("body").prepend(this.$ruler.bottom);
            
             $("body").mousemove(function(mouse){
               $("#ruler-top").css("height", mouse.pageY - parseInt(60) + "px");         
               $("#ruler-bottom").css("top", mouse.pageY + parseInt(60) + "px");
         });
            
         /*window.addEventListener('mousemove', function(e) {
               $("#ruler-top").css("height", e.clientY - parseInt(60) + "px");         
               $("#ruler-bottom").css("top", e.clientY + parseInt(60) + "px");       
            });*/
        ...
        $(".icon-text").on("click", function () {
            ...
            var seuIFrame = (document.getElementById("book_frame").contentWindow || document.getElementById("book_frame").contentDocument);
            if (seuIFrame.document) {
                seuIFrame = seuIFrame.document;
            }
  
            var text;
          if(seuIFrame.getSelection) {
                text = String(seuIFrame.getSelection());
            }else if(seuIFrame.selection){
                text = seuIFrame.selection.createRange().text;
            }
            
            if(text != ''){
                responsiveVoice.speak(text, "Brazilian Portuguese Female", {rate: 10.0});
            }else{
                alert("Nenhum trecho do texto foi selecionado!");
            }
        ...
        Settings.getMultiple(['reader', ebookURL_filepath], function(settings){
            ...
            ReadiumSDK.on(ReadiumSDK.Events.PLUGINS_LOADED, function () {
                ...
                if (!readium.reader.plugins.highlights) {
                    ...
                    $('.icon-annotations').css("display", "block");
